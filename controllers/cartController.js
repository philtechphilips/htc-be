...
exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    if (cartRows.length) {
      await schema.update("cart", { user_id, product_id }, { quantity: cartRows[0].quantity + quantity });
    } else {
      await schema.create("cart", { id: uuidv4(), user_id, product_id, quantity });
    }
    return successResponse(res, { statusCode: 200, message: "Product added to cart", payload: null });
  } catch (err) {
    console.error(err);
...
exports.removeFromCart = async (req, res) => {
  ...
  try {
    ...
    return successResponse(res, { statusCode: 200, message: "Product removed from cart", payload: null });
  } catch (err) {
    return errorResponse(res, { statusCode: 500, message: "Server error" });
  }
};
