...
exports.register = async (req, res) => {
  ...
  try {
    ...
    const id = uuidv4();
    await schema.create('users', { id, firstName, lastName, email, password: hashedPassword });
    const newUser = await schema.fetchOne('users', { id });
    if (newUser && newUser.password) delete newUser.password;
    return successResponse(res, { statusCode: 201, message: 'User registered', payload: newUser });
  } catch (err) {
    console.error(err);
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    return successResponse(res, { statusCode: 200, message: 'Login successful', payload: userWithoutPassword, token });
  } catch (err) {
    return errorResponse(res, { statusCode: 500, message: 'Server error' });
  }
};
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return errorResponse(res, { statusCode: 500, message: 'Email service error' });
    }
